...
def get_single_problem(parsed_html: bs4.BeautifulSoup):
    """
    Get the problem from the html with parsed by bs4
    """
    question = {
...
def get_problems_from_leetcode(url: str, rg_list: list) -> list:
    """
    :param url: 
    :param rg: limit get
    :return: 
    ...
    if request_flag:
        ...
        questions_groups = (json_data['props']['pageProps']['dehydratedState']
        ['queries'][0]['state']['data']['studyPlanV2Detail']['planSubGroups'])
        for question_group in questions_groups:
            for questions in question_group['questions']:
                u = questions['titleSlug']
        ...
        write_to_json(qs, 'questions.json')


def write_to_json(questions_list: list, filename: str):
